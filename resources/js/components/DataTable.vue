...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            defaultOrder () {
                let keys = this.columns.map(col => col.data);
                let order = this.order.filter( item => this.visibleColumns.indexOf(item.key) !== -1 );
        ...
        methods: {
            ...
            initTable() {
                let table = $(this.$refs.theTable).DataTable( {
                    language: {
        ...
        mounted() {
            ...
            table.on( 'init', (event) => {
                ...
                let $tableWrapper = $(table.containers()[0]),
                    $tableHeader = $tableWrapper.find('> .row:first');

                // Swap the columns, so that the empty cell comes first.
                // We will use this for our column selector.
                let $container = $tableHeader.find('> div:last').detach();
